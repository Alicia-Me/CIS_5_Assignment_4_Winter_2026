...
int main() {
    int abc[10] = { 75, 88, 62, 95, 50, 82, 70, 91, 45, 78 };
    int size = 10;
    ...
    int sum = calculateSum(abc, size);
    double avg = calculateAverage(sum, size);
    cout << "Sum: " << sum << endl;
    cout << "Average: " << fixed << setprecision(2) << avg << endl;

    addBonus(abc, size, 10);
    cout << "Scores after +10 bonus: ";
    for (int i = 0; i < size; i++) cout << abc[i] << " ";
    cout << endl;

    int minScore = findMinimum(abc, size);
    cout << "Minimum score: " << minScore << endl;

    int highPerformers = countHighPerformers(abc, size);
